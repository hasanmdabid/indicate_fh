...
for model_name in models:
    if model_name == 'xception':
        ...
        size = 299
        pretrained_model = Xception(weights='imagenet', input_shape=(size, size, n_channels), include_top=False)
    elif model_name == 'mobilenetv2':
        from keras.applications.mobilenet_v2 import MobileNetV2
        size = 224
        pretrained_model = MobileNetV2(weights='imagenet', input_shape=(size, size, n_channels), include_top=False)
    elif model_name == 'vgg16':
        from keras.applications import VGG16
        size = 224
        # Load the pre-trained VGG16 model
        pretrained_model = VGG16(weights='imagenet', input_shape=(size, size, n_channels), include_top=False)
    elif model_name == 'inceptionv3':
        from keras.applications.inception_v3 import InceptionV3
        size = 299
        pretrained_model = InceptionV3(weights='imagenet', input_shape=(size, size, n_channels), include_top=False, )
